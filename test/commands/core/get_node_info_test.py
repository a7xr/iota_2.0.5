...
class GetNodeInfoRequestFilterTestCase(BaseFilterTestCase):
  ...
  def test_pass_empty(self):
    ...
    print_var_type_n_val(var001 = filter_, pointer = "#SDFGHhgfd123456666DFFdd")#SDFGHhgfd123456666DFFdd
# Value: 
# # GetNodeInfoRequestFilter(FilterChain(Type(Mapping, allow_subclass=True) | FilterMapper()))
...
class GetNodeInfoResponseFilterTestCase(BaseFilterTestCase):
  ...
  def test_pass_happy_path(self):
    ...
    # "def _filter()" below is going to be defined in 
    # # site-packages/filters/test.py
    print_var_type_n_val(var001 = filter_, pointer = "#SDFGHhgfBVCdfg1234555")#SDFGHhgfBVCdfg1234555
# Value: 
# # GetNodeInfoResponseFilter(FilterChain(Type(Mapping, allow_subclass=True) | FilterMapper(latestMilestone=FilterChain(ByteString(encoding='ascii') | Trytes()), latestSolidSubtangleMilestone=FilterChain(ByteString(encoding='ascii') | Trytes()))))
